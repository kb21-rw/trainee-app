...
import ResponseModal from "../../components/modals/ResponseModal";
 const  TraineeResults = () => {
   ...
   const { data, isFetching, isError } = useGetOverviewForCoachQuery({ jwt });
      ...
      const questionType = question.options.length > 0 ? "dropdown" : "text";
       question.responses.forEach((response:Response) => {
         if (response.user) {
          ...
          if(traineeInfo.type === "text") {
            traineeInfo.responses[`${form._id}-${question._id}`] = response.text ?? "No response";
          } else if(traineeInfo.type === "dropdown") {
            traineeInfo.responses[`${form._id}-${question._id}`] = question.options.map((option) => option.title)[parseInt(response.text ?? "0")];
          }
         }
       });
     });
         ...
         <TableBody className="bg-white divide-y divide-gray-300">
           {Array.from(traineeMap.values()).map((trainee) => (
             <TableRow key={trainee.name}>
               ...
               {data.flatMap((form) => form.questions.map((question:Question) => (
                 <TableCell key={`${form._id}-${question._id}`} className="border border-black p-2 w-16 max-w-md overflow-auto whitespace-nowrap"
                 onClick={() => {
                  setIsModalOpen(true);
                  setModalData({
